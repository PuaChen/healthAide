...
public class HUserIndexServiceImpl extends ServiceImpl<HUserIndexMapper, HUserIndex> implements HUserIndexService {
    ...
    public JsonResult<HIllness> createRecord(HUserIndex index) {
        ...
        wrapper.eq("illness_name", illnessType);
        HIllness illness = illnessService.getOne(wrapper);
        if (illness == null) {
            log.error("没有找到这种疾病类型 :{}", illnessType);
            return JsonResult.error("不存在的疾病类型");
        }
        index.setIllnessId(illness.getId());
        index.setUserId(ContextUtil.getUserId());
    ...
    @Override
    public List<IndexDataVo> getDataList(String startTime, String endTime) {
        return baseMapper.selectDataList(startTime, endTime, ContextUtil.getUserId(), null);
    }
     ...
     */
    @Override
    public IndexDataVo getDataForDay(String dayTime) {
        return baseMapper.getDataForDay(dayTime, ContextUtil.getUserId());
    }
